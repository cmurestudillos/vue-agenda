  ...
  <div id="modificar-contacto">
    ...
    <div class="tarjeta">
      <div class="cabecera-tarjeta">
        ...
        <div class="iconos">
          <img src="../assets/more_vert-black-24dp.svg" alt="Más opciones" @click="editarTarjeta">
          <img src="../assets/clear-black-24dp.svg" alt="Salir" @click="$emit('cerrar-tarjeta')">
        </div>
      </div>
      <div class="form__group field">
        <input type="text" class="form__field" placeholder="Nombre" name="nombreModifica" v-model="nombreModifica" :class="nombreModifica !== '' ? 'numeroCorrecto' : 'numeroIncorrecto'" :disabled="!editarContacto" id='nombreModifica' />
        <label for="nombreModifica" class="form__label"> Nombre </label>
      </div>
      <p v-if="errorNombre !== ''" class="error"> {{ errorNombre }} </p>
      <div class="form__group field">
        <input type="text" class="form__field" placeholder="Nombre" name="telefonoModifica" v-model="telefonoModifica" :class="!isNaN(telefonoModifica) && telefonoModifica !== '' ? 'numeroCorrecto' : 'numeroIncorrecto'" :disabled="!editarContacto" id='telefonoModifica' />
        <label for="telefonoModifica" class="form__label"> Teléfono </label>
      </div>
      <p  v-if="errorTelefono !== ''" class="error"> {{ errorTelefono }} </p>
      <div class="grupo-botones">
        <a v-if="editarContacto" class="boton eliminar" @click="confirmacionEliminar = true"> Eliminar </a>
        <a v-if="editarContacto" class="boton guardar" @click="checkForm"> Guardar </a>
      </div>
      <div v-if="confirmacionEliminar" class="confirmar-eliminar">
        <p class="texto-eliminar">¿Estás seguro que quieres eliminar el contacto?</p>
        <a @click="confirmacionEliminar = false" class="confirmacion-volver"> Volver </a>
        <a @click="$emit('eliminar-contacto', contacto.id)" class="confirmacion-eliminar"> Eliminar contacto </a>
      </div>
    </div>
  </div>
</template>
